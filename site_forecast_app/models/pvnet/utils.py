...
def process_and_cache_nwp(nwp_config: NWPProcessAndCacheConfig) -> None:
    ...
    if nwp_config.source == "ecmwf" and expand_ecmwf:
        ...
        log.info("Expanding ECMWF data by 6 hours into the future "
                 "and adding an extra 0.5 degree down")

        # expand time by 6 hours
        for i in range(6):
            new_step_value = ds.step.values[-1] + 3600000000000
            new_step = ds.interp(step=new_step_value, method='linear', kwargs={'fill_value': 'extrapolate'})
