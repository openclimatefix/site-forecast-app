...
def test_satellite_scale_minmax(
    small_satellite_data,  # noqa: ARG001
) -> None:
...
def test_satellite_download(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        download_satellite_data(
            satellite_source_file_path=os.getenv("SATELLITE_ZARR_PATH"),
            local_satellite_path=local_satellite_path,
            scaling_method="minmax",
        )


def test_satellite_download_backup(
    small_satellite_data,  # noqa: ARG001
) -> None:
    """Test for scaling satellite data using min-max scaling."""
