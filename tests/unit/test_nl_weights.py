...
async def test_get_nl_blend_weights_missing_init_times():
    """Verify weights calculation when models are missing fallback to penalty calculations correctly."""
...
async def test_get_nl_blend_weights_all_fail():
    """Verify fallback when no initialisation times exist, everything falls back to backup if necessary."""
    t0 = pd.Timestamp("2024-06-01 12:00", tz="UTC")
